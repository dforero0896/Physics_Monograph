...
void calculateOperator(double nEnergy, double dist, double dens, gsl_matrix_complex *container){
	...
	double theta2 = 5.; //Degrees
	double theta3 = 45.; //Degrees
	...
	//Energy differences.
	E21=energies(dm21, nEnergy);
	E32=energies(dM32, nEnergy);
	...
	long double s1 = pow((r+sqrt(q*q*q+r*r)), 1./3);
	long double s2 = pow((r-sqrt(q*q*q+r*r)), 1./3);
	/*This throws nan's for some reason
	gsl_complex lam1 = gsl_complex_add(real2comp(-0.5*(s1+s2)), gsl_complex_mul(gsl_complex_rect(0, 1.), real2comp((sqrt(3.))/2*(s1-s2))));
	gsl_complex lam2 = gsl_complex_add(real2comp(-0.5*(s1+s2)), gsl_complex_mul(gsl_complex_rect(0, 1.), real2comp((-sqrt(3.)/2)*(s1-s2))));
	gsl_complex lam3 = real2comp(s1+s2);
	*/
	...
	traceHm=0.5*E21+E32+3*nEnergy+A;
